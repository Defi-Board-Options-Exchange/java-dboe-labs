...
@Name('DboeInstr') select instr_id, chain, collateral_group, underlying, kind, expiry, ltt, ltt_utc, strike, cond_strike, multiplier, currency, long_contract_address, short_contract_address from DboeOptionInstrWin output snapshot at (*/15, *, *, *, *)
...
@Name('DboeGiftOpenWin') select name, wallet_id, open_key, reward, 'OLD' as status, current_timestamp() as timestamp from DboeMGUserOpenWin(status='NEW')
